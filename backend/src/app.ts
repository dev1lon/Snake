...
// Nonces live in the database when there is one. On Render that is merely
// tidier; on a serverless platform it is the difference between working and
// not, because the instance that issues a nonce is rarely the one that
...
async function issueNonce() {
  ...
  await pool.query("INSERT INTO auth_nonces (nonce) VALUES ($1)", [nonce]);
  return nonce;
}

// Claims a nonce for good. The delete and the read are one statement, so two
// simultaneous replays of the same signature cannot both win.
async function claimNonce(nonce: string) {
  if (!pool) {
    const createdAt = nonces.get(nonce);
    nonces.delete(nonce);

    return Boolean(createdAt && Date.now() - createdAt <= nonceMaxAgeMs);
  }

  const result = await pool.query<{ fresh: boolean }>(
    `DELETE FROM auth_nonces
      WHERE nonce = $1
      RETURNING created_at > NOW() - make_interval(secs => $2) AS fresh`,
    [nonce, nonceMaxAgeMs / 1000]
  );
...
async function cleanupNonces() {
  ...
  await pool.query("DELETE FROM auth_nonces WHERE created_at < NOW() - make_interval(secs => $1)", [
    nonceMaxAgeMs / 1000
  ]);
}
